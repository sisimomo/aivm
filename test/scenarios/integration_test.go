...
func TestIntegrationRunsForActiveAgent(t *testing.T) {
	...
	h.Scenario("rtk→claude integration runs when rtk installed and claude active").
		...
		Assert("rtk:claude integration recorded in bootstrap state",
...
func TestIntegrationSkipsForInactiveAgent(t *testing.T) {
	...
	h.Scenario("rtk→claude integration skipped when copilot is active").
		...
		Wait("VM is running", conditions.VMStatus(vm.StatusRunning), 5*time.Minute).
		Assert("Bootstrap complete", assertions.BootstrapComplete()).
		Assert("rtk:copilot integration recorded in bootstrap state",
			assertions.BootstrapStateContainsIntegrations("rtk:copilot")).
...
func TestIntegrationNotRunWithoutPlugin(t *testing.T) {
	...
	h.Scenario("rtk integration skipped when rtk is not installed").
		...
		Wait("VM is running", conditions.VMStatus(vm.StatusRunning), 5*time.Minute).
		Assert("Bootstrap complete", assertions.BootstrapComplete()).
		Assert("mcp:claude integration recorded (agent-only integration always runs)",
			assertions.BootstrapStateContainsIntegrations("mcp:claude")).
		Assert("rtk:claude not recorded (rtk absent)",
			assertions.BootstrapStateNotContainsIntegrations("rtk:claude")).
		Assert("rtk→claude marker file is absent in VM",
			assertions.VMFileAbsent("/tmp/.aivm_test_integ_rtk_claude")).
		Run()
}
...
// second start. The marker file still exists, but the VM run count stays at 0
// after the second start (no scripts re-executed).
func TestIntegrationIsIdempotent(t *testing.T) {
	...
	h.Scenario("integration runs once and is idempotent on second start").
		...
		Wait("VM is running", conditions.VMStatus(vm.StatusRunning), 5*time.Minute).
		Assert("Bootstrap complete", assertions.BootstrapComplete()).
		Assert("rtk:claude integration recorded", assertions.BootstrapStateContainsIntegrations("rtk:claude")).
		...
		Assert("No scripts ran on second start (integration was already tracked)", assertions.VMRunCountIs(0)).
		Assert("rtk:claude integration still recorded in state",
			assertions.BootstrapStateContainsIntegrations("rtk:claude")).
		Run()
}
...
func TestIntegrationRunsWhenPluginNewlyInstalled(t *testing.T) {
	...
	h.Scenario("rtk integration runs when rtk is added after initial boot").
		...
		Wait("VM is running", conditions.VMStatus(vm.StatusRunning), 5*time.Minute).
		Assert("Bootstrap complete", assertions.BootstrapComplete()).
		Assert("mcp:claude recorded on first boot (agent-only)",
			assertions.BootstrapStateContainsIntegrations("mcp:claude")).
		Assert("rtk:claude not yet recorded (rtk absent on first boot)",
			assertions.BootstrapStateNotContainsIntegrations("rtk:claude")).
		...
		Assert("VM still running", assertions.VMStatus(vm.StatusRunning)).
		Assert("Scripts ran: rtk install + integration", assertions.VMRunCountAtLeast(1)).
		Assert("rtk:claude integration now recorded", assertions.BootstrapStateContainsIntegrations("rtk:claude")).
