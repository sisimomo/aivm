...
| `aivm agent -- <args>` | Start VM + services, then run the agent CLI with custom arguments (skips `launch_args`; `--` required) |
...
|------------|-------------------|-----------------|
| `aivm` (default) | Yes | From config (e.g. `--dangerously-skip-permissions` for Claude) |
...
```bash
# Cursor — set as the custom agent command prefix (Cursor appends its flags after --)
...
[Context7](https://context7.com) provides up-to-date library documentation for AI coding agents. When enabled, aivm installs the `ctx7` CLI and the `find-docs` skill for every supported agent via `npx skills@latest`.
...
No API key required — `ctx7 library` and `ctx7 docs` work without authentication. For higher rate limits, set `CONTEXT7_API_KEY` via `vm.env` (supports `${HOST_VAR}` expansion from your host shell):
