...
`vm.session_env` maps environment variable names to values, using the same `${HOST_VAR}` expansion as `vm.env`. On each interactive entry (`aivm` or `aivm ssh`), values are resolved from the invoking host process and exported inside the VM for that session only.
